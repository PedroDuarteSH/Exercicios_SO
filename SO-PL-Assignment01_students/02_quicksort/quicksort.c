...
int quicksort(int s[], int l, int h){
  ...
  if ((h-l) > 0){					        /* If the number of member is higher than 1 */
    p = partition(s, l, h-1);			  /*  finds the index for the partition */
    quicksort(s, l, p-1);			    /*  and sorts the lower values */
    quicksort(s, p, h);			      /*  and sorts the higher values */
  }
  return 0;
}
int main(void){
  ...
  /* Sort array */
  quicksort(x,0, sizeof(x)/sizeof(int));    /* Orders array using quicksort() */
